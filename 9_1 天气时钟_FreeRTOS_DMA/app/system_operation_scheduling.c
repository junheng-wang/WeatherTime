...
#define SCHEDULINGLOGIC_EVENT_TIME_UPDATE		(1 << 2)
#define SCHEDULINGLOGIC_EVENT_INNER_UPDATE		(1 << 3)
#define SCHEDULINGLOGIC_EVENT_OUTDOOR_UPDATE	(1 << 4)
#define SCHEDULINGLOGIC_EVENT_ALL				(SCHEDULINGLOGIC_EVENT_TIME_SYNC   |\
												 SCHEDULINGLOGIC_EVENT_WIFI_UPDATE |\
												 SCHEDULINGLOGIC_EVENT_TIME_UPDATE |\
...
void systemSchedulingLogic(void* param)
{
	...
	while(1)
	{
		...
		if(event & SCHEDULINGLOGIC_EVENT_TIME_UPDATE)
...
void creatSystemSchedulingLogic(void)
{
	...
	time_update_timer = xTimerCreate("time update", pdMS_TO_TICKS(TIME_UPDATE_INTERVAL), pdTRUE, (void*)SCHEDULINGLOGIC_EVENT_TIME_UPDATE, allTimerCallback);
