...
static volatile AtAckType rx_ack;

// 定义一个信号量
// static SemaphoreHandle_t esp_at_dma_semaphore;
static SemaphoreHandle_t esp_at_usart_semaphore;


void initUsart2(void);
void initDma1Stream6(uint8_t* memory_addr, uint32_t len);
void sendDataToEsp32ByUsart2(const char* data);
void sendDataToEsp32ByUsart2UseDma(const char* data);
AtAckType receiveAndRecognizeResponseMsgFromEsp32ByUsart2(uint32_t timeout_ms);
AtAckType receiveAndRecognizeResponseMsgFromEsp32ByUsart2Semphr(uint32_t timeout_ms);
...
{
	// 0. 初始化二值信号量
	// esp_at_dma_semaphore = xSemaphoreCreateBinary();
	esp_at_usart_semaphore = xSemaphoreCreateBinary();
...
void initUsart2(void)
{
	...
	USART_DMACmd(USART2, USART_DMAReq_Tx, ENABLE);	// 发送使用DMA
...
/**
 *@brief DMA1_Stream6初始化，用于USART2与ESP32通信
...
/**
 *@brief MCU通过USART2向ESP32发送数据(DMA传输)
 *@param data 要发送的数据
 */
void sendDataToEsp32ByUsart2UseDma(const char* data)
{
	uint32_t len = strlen(data);
	initDma1Stream6((uint8_t*)data, len);
...
AtAckType receiveAndRecognizeResponseMsgFromEsp32ByUsart2(uint32_t timeout_ms)
{
	...
	uint64_t start_ms = (uint64_t)xTaskGetTickCount();
    while (rx_len < sizeof(rx_buf) - 1)
    {
        while (USART_GetFlagStatus(USART2, USART_FLAG_RXNE) == RESET)
        {
            if((uint64_t)xTaskGetTickCount() - start_ms >= timeout_ms)
...
/**
 *@brief 接收ESP32返回给MCU的响应消息，USART2使用信号量
 *@param timeout_ms 超时退出时间，单位：毫秒
 *@return ESP32返回的响应消息类型
 */
AtAckType receiveAndRecognizeResponseMsgFromEsp32ByUsart2Semphr(uint32_t timeout_ms)
{
	...
	line = rx_buf;
	// 获取二值信号量，获取不到就阻塞
	if(xSemaphoreTake(esp_at_usart_semaphore, pdMS_TO_TICKS(timeout_ms)) == pdPASS)
		return rx_ack;
...
uint8_t sendCommandAndReceiveRespondMsg(const char* command, uint32_t timeout_ms)
{
	...
	// 发送AT指令
	sendDataToEsp32ByUsart2UseDma(command);
	// 接收响应消息
	AtAckType at_ack = receiveAndRecognizeResponseMsgFromEsp32ByUsart2Semphr(timeout_ms);
...
uint8_t waitReadyResponseMsg(uint32_t timeout_ms)
{
	return receiveAndRecognizeResponseMsgFromEsp32ByUsart2Semphr(timeout_ms) == AT_ACK_READY;
}
		...
		if (rx_len < sizeof(rx_buf) - 1)
		{
			...
			if(rx_buf[rx_len-1] == '\n')
			{
				...
				if(at_ack != AT_ACK_NONE)
				{
					...
					// 外设的中断给任务发通知，基本上都是使用信号量的方法，很少使用任务通知的方法
					// 因为可能很多任务都会涉及到这个外设的中断，但是任务通知只能给指定的任务发送通知
					xSemaphoreGiveFromISR(esp_at_usart_semaphore, &xHigherPriorityTaskWoken);
...
//		// 外设的中断给任务发通知，基本上都是使用信号量的方法，很少使用任务通知的方法
//		// 因为可能很多任务都会涉及到这个外设的中断，但是任务通知只能给指定的任务发送通知
//		xSemaphoreGiveFromISR(esp_at_dma_semaphore, &xHigherPriorityTaskWoken);
//		portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
	}
}
