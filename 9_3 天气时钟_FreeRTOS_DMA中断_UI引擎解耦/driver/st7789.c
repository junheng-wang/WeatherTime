...
// 定义一个信号量
static SemaphoreHandle_t st7789_semaphore;
...
{
	// 0. 初始化二值信号量
	st7789_semaphore = xSemaphoreCreateBinary();
...
void writeDataToSt7789UseDma(uint8_t data[], uint32_t len, uint8_t memory_inc)
{
	...
	while(temp_len > 0)
	{
		...
		// 获取二值信号量，获取不到就阻塞
		xSemaphoreTake(st7789_semaphore, portMAX_DELAY);
...
void DMA1_Stream4_IRQHandler(void)
{
	if(DMA_GetITStatus(DMA1_Stream4, DMA_IT_TCIF4) == SET)
	{
		...
		xSemaphoreGiveFromISR(st7789_semaphore, &xHigherPriorityTaskWoken);
